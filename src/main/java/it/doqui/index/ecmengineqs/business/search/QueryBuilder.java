...
public class QueryBuilder {
    ...
    private enum QueryType {
        NORMAL, WILDCARD, RANGE
    };

    /**
     * Costruttore protected definito per nascondere il costruttore pubblico di
     * default poich&eacute; la classe non deve essere istanziabile.
     */
    ...
    /**
     * Costruisce la query Lucene a partire dai parametri di ricerca passati in
     * input.
     *
...
//		return LuceneQueryParser.escape(input).replace("\\*", "*");
     ...
     *
     * <p>
     * L'escaping &egrave; effettuato scomponendo il path in elementi e codificando
     * ogni elemento corrispondente ad un QName completo in formato ISO9075.
     * </p>
     *
    ...
    public static String escapeXPathQuery(String input) {
        ...
        for (int i = startIndex; i < elements.length; i++) {
            if (elements[i].length() == 0 || elements[i].contains("*")) {
            ...
            } else if (!elements[i].contains(":") && !elements[i].equalsIgnoreCase("node()")) {
                // Default namespace
                ...
                output.append('/').append(parts[0]).append(":")
                    .append((parts.length > 1) ? ISO9075.encode(parts[1]) : "");
            } else {
                output.append('/').append(elements[i]);
            }
        }
     ...
     *
     * <p>
     * L'escaping &egrave; effettuato riconoscendo nella stringa i termini
     * &quot;PATH&quot; e i termini &quot;QNAME&quot;: ai sono applicate le medesime
     * regole valide per l'escaping XPath, i secondi vengono codificati in formato
     * ISO9075.
     * </p>
     *
     * <p>
     * <strong>NB:</strong> questo metodo effettua un controllo di validit&agrave;
     * sul numero di doppi apici trovati nella stringa. Gli eventuali errori sono
     * segnalati sollevando una {@code IllegalArgumentException}.
     * </p>
     *
    ...
    public static String escapeLuceneQuery(String input) {
        ...
        int escapeQuotesCount = countOccurrences("\\\"", input);

        if ((quotesCount - escapeQuotesCount) % 2 == 1) {
    ...
    private static String escapeLuceneQNameTermsInQuery(String input) {
        ...
        output.append(input, 0, qnameBegin);
        ...
        while (qnameBegin >= 0) {
            ...
            output.append(input, qnameBegin, qnameBegin + 7).append(escapedQName);

            qnameBegin = input.indexOf("QNAME:\"", qnameEnd + 1);
            if (qnameBegin == -1) {
                output.append(input.substring(qnameEnd));
            } else {
                output.append(input, qnameEnd, qnameBegin);
            }
        }
